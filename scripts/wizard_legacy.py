...
""")
                ...
                targets = ["ExfinApiService", "Exfin_ApiService", current_svc_name]
                if current_svc_name != "Exfin_ApiService":
                    targets = ["ExfinApiService", "Exfin_ApiService", current_svc_name] # Ensure default is always checked
    ...
    def create_step_finish(self):
        ...
        summary = ("Tebrikler! EXFIN API kurulumu başarıyla tamamlandı.\n\n"
                   ...
                   "• Windows Servisi (Exfin_ApiService) kuruldu ve başlatıldı.\n\n"
    ...
    def finish_setup(self):
        ...
        # 2. Save to api.db (Single Source of Truth)
        try:
            import sqlite3
            sql_conn = sqlite3.connect("api.db")
            sql_cur = sql_conn.cursor()
            sql_cur.execute("CREATE TABLE IF NOT EXISTS settings (key TEXT PRIMARY KEY, value TEXT)")
            
            # Updated Schema to match migrate_to_sqlite.py
            sql_cur.execute('''CREATE TABLE IF NOT EXISTS db_connections 
            ...
            """, (
                "Postgres_Main", 
                "PostgreSQL", 
                self.config_data.get("pg_host"), 
                int(self.config_data.get("pg_port", 5432)), 
                self.config_data.get("pg_db"), 
                self.config_data.get("pg_user"), 
                self.config_data.get("pg_pass")
            ))
            ...
            """, (
                "LOGO_Database", 
                "MSSQL", 
                self.config_data.get("ms_host"), 
                1433, # Default MSSQL port if not specified
                self.config_data.get("ms_db"), 
                self.config_data.get("ms_user"), 
                self.config_data.get("ms_pass")
            ))
            ...
            messagebox.showerror("Hata", f"Veritabanı yapılandırması kaydedilemedi: {e}")

        # Removed db_config.json writing logic per user request
